...
namespace Panda
{
    ...
    {
        // Make sure lambda array is sorted.
        for (int32_t i = 0; i < n - 1; ++i) assert(lambda[i + 1] > lambda[i]);
        assert(lambdaEnd > lambdaStart);
        ...
        auto interp = [lambda, vals](float w, int32_t i) {
    ...
    SampledSpectrum SampledSpectrum::FromRGB(const float rgb[3], SpectrumType type)
    {
        ...
        if (type == SpectrumType::Reflectance)
        {
            ...
            {
                // Compute reflectance _SampledSpectum_ with _rgb[0]_ as minimum
                                            ...
                                            int32_t n, float l)
    {
        ...
        int32_t offset = FindInterval(n, [&](int32_t index) {return lambda[index] <= 1;});
