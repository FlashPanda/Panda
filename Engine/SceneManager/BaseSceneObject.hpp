...
namespace Panda
{
    ...
    {
        kSceneObjectTypeInteraction = "ISEC"_i32,
        kSceneObjectTypeMath = "MATH"_i32,
    ...
    protected:
        ...
        BaseSceneObject(SceneObjectType type, std::string name = "") : m_Type(type) { m_Guid = newGuid(); }
        BaseSceneObject(Guid& guid, SceneObjectType type, std::string name = ""): m_Guid(guid), m_Type(type) {}
        BaseSceneObject(Guid&& guid, SceneObjectType type, std::string name = "") : m_Guid(std::move(guid)), m_Type(type) {}
        BaseSceneObject(BaseSceneObject&& obj): m_Guid(std::move(obj.m_Guid)), m_Type(obj.m_Type), m_Name(obj.m_Name) {}
        BaseSceneObject& operator=(BaseSceneObject&& obj){this ->m_Guid = std::move(obj.m_Guid); this->m_Type = obj.m_Type; this->m_Name = obj.m_Name; obj.m_Guid = Guid("0"); obj.m_Name = ""; return *this;}
